...
class GhostModule(nn.Cell):
    def __init__(self, in_channel: int,
        ...
        self.primary_conv = Conv2dNormActivation(in_channel, init_channels, kernel_size=kernel_size, stride=stride, padding=0,
...
class GhostBottleneck(nn.Cell):
    def __init__(self, in_channel: int,
        ...
        self.num_in = in_channel
        self.ghost1 = GhostModule(in_channel, mid_channel, kernel_size=1, stride=1, pad_mode=pad_mode,
        ...
        self.dw = None
        if self.use_dw:
            self.dw = Conv2dNormActivation(mid_channel, mid_channel, kernel_size=kernel_size, stride=stride,
                                           pad_mode=pad_mode, groups=mid_channel, norm=norm, activation=None)

        # Squeeze-and-excitation
        self.use_se = use_se
        if use_se:
            self.se = SqueezeExcite(mid_channel, rd_ratio=1./4, rd_divisor=4, act_layer=nn.ReLU(), gate_layer=GhostGate())

        # Point-wise linear projection
        self.ghost2 = GhostModule(mid_channel, out_channel, kernel_size=1, stride=1,
        ...
        if self.down_sample:
            self.shortcut = nn.SequentialCell([
                Conv2dNormActivation(in_channel, in_channel, kernel_size=kernel_size, stride=stride,
                                     pad_mode=pad_mode, groups=in_channel, norm=norm, activation=None),
                Conv2dNormActivation(in_channel, out_channel, kernel_size=1, stride=1, padding=0,
