...
class DualPathBlock(nn.Cell):
    def __init__(self,
        ...
        if self.has_proj:
            ...
            self.c1x1_w_r = self.Conv1x1(in_chs=in_channel, out_chs=num_1x1_c, stride=key_stride)
            self.c1x1_w_d = self.Conv1x1(in_chs=in_channel, out_chs=2 * inc, stride=key_stride)

        self.layers = BottleBlock(in_channel, num_1x1_a, num_3x3_b, num_1x1_c, inc, G, key_stride)

    def Conv1x1(self, in_channel, out_channel, stride):
        return nn.Conv2d(in_channel, out_channel, kernel_size=1, stride=stride, pad_mode='pad', padding=0)
