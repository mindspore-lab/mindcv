...
class Mlp(nn.Cell):
    ...
    def __init__(self,
        ...
        self.fc1 = nn.Dense(in_channels=in_channel, out_channels=hidden_channel, has_bias=True)
        self.act = act_layer()
        self.fc2 = nn.Dense(in_channels=hidden_channel, out_channels=out_channel, has_bias=True)
...
class RelativeBias(nn.Cell):
    ...
    def construct(self, axis: int = 0) -> Tensor:
...
class SwinTransformerBlock(nn.Cell):
    ...
    def __init__(self,
        ...
        self.mlp = Mlp(in_channel=dim[0] if isinstance(dim, tuple) else dim, hidden_channel=mlp_hidden_dim,
