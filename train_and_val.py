...
from mindcv.utils import ValAccSaveMonitor
...
def train(args):
    ...
    ckpt_cb = ModelCheckpoint(prefix=args.model,
                              ...
                              config=ckpt_config)
    val_cb = ValAccSaveMonitor(model, loader_eval, metric_name="Top_1_Accuracy", ckpt_dir=args.ckpt_save_dir,
                               best_ckpt_name=args.model + '_best.ckpt', dataset_sink_mode=args.dataset_sink_mode)
    if args.distribute:
        if rank_id == 0:
